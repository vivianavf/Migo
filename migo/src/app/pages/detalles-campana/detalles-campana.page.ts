...
export class DetallesCampanaPage implements OnInit {
  ...
  generarDatos() {
    ...
    puertas.forEach((puerta) => {
      const parteExiste = this.partesBrandeables.find(
        (parte) => parte.nombre === puerta.nombre
      );
      if (puerta.valor !== 0.0 && !parteExiste) {
        switch (puerta.nombre) {
          case 'carroceria_capo':
            this.partesBrandeables.push({
              nombre: 'Capó',
              costo: puerta.valor,
            });
            break;
          ...
          case 'puerta_trasera_iz':
            this.partesBrandeables.push({
              nombre: 'Puerta trasera izq.',
              costo: puerta.valor,
            });
            break;
          case 'puerta_trasera_der':
            this.partesBrandeables.push({
              nombre: 'Puerta trasera der.',
              costo: puerta.valor,
            });
            break;
  ...
  getBannerURL(campana: Campana) {
    ...
    return this.empresaImagesService.getBannerURLbyEmpresaId(
      idEmpresa,
      this.imagesEmpresas
    );
  }
  ...
  crearSectores() {
    this.sectorService.getSectores().subscribe((data) => {
      console.log('sectores lengh, ', this.sectores.length);
      console.log('data, ', data);
      this.sectores = data;
      this.campana = this.campanaService.getCampanaActual();
      data.forEach((sectorObtenido) => {
        console.log('sector obtenido del data', sectorObtenido);
        if (
          this.campana.id_sector == sectorObtenido.id_sector &&
          sectorObtenido.id_campana == this.campana.id_campana
        ) {
          this.sector = sectorObtenido;
          console.log(
            'this.sector, sector obtenido',
            this.sector,
            sectorObtenido
          );
        }
  ...
  createMap() {
    ...
    console.log(this.sector?.cerco_virtual);
    this.googleMapService.createMap(
      this.sector!.centro,
      this.sector!.zoom,
      'map-campana',
      this.mapDetalleCampana!,
  ...
  ionViewDidLeave() {
    ...
    this.clientes = [];
    this.sectores = [];
    this.talleresBrandeo = [];
    this.vehiculosAdmisibles = [];
    this.nombreEmpresa = '-';
